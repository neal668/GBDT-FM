...
class FMWithSGD(private var task: Int,
    ...
    (k0, k1) match {
      case (true, true) =>
        Vectors.dense(Array.fill(numFeatures * k2)(Random.nextGaussian() * initStd + initMean) ++
          Array.fill(numFeatures + 1)(0.0))   // k0 - w0, k1 - w1, k2 - interactions
  ...
  private def createModel(weights: Vector): FMModel = {
    ...
    val v = new DenseMatrix(k2, numFeatures, values.slice(0, numFeatures * k2))  // build a Dense Matrix
  ...
  def run(input: RDD[LabeledPoint]): FMModel = {
    ...
    val weights = optimizer.optimize(data, initWeights)  // optimization
